    ...
    <AppLayout>
      ...
      <PageContent>
        ...
        <Card glow className="p-6 flex flex-col items-center text-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-[#7A7A8C]">
              {metrics.waterMl}ml of {metrics.waterGoalMl}ml daily goal
            </p>
            {pct >= 100 ? (
              <p className="text-[#30D158] font-bold mt-1">🎉 Goal Reached!</p>
            ) : (
              <p className="text-[#7A7A8C] text-xs mt-1">
        ...
        {/* Quick add buttons */}
        <div>
          <p className="text-xs font-bold text-[#7A7A8C] uppercase tracking-wider mb-3">Quick Add</p>
          <div className="grid grid-cols-4 gap-3">
            {glasses.map(({ label, icon, ml }) => (
              <button
                key={label}
                onClick={() => addWater(ml)}
                className="flex flex-col items-center gap-2 p-4 bg-[#17171A] border border-[#2A2A30] rounded-2xl hover:border-[#0A84FF]/50 hover:bg-[#0A84FF]/5 transition-all active:scale-95"
              >
                <span className="text-3xl">{icon}</span>
                <p className="text-xs font-semibold text-[#F0F0F5]">{label}</p>
                <p className="text-[10px] text-[#7A7A8C]">{ml}ml</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* 7-day history */}
        <Card className="p-5">
          <p className="text-xs font-bold text-[#7A7A8C] uppercase tracking-wider mb-4">7-Day History</p>
          <div className="flex items-end gap-2 h-28">
            {last7.map(({ label, waterMl, goalMl }) => {
              const barPct = Math.min((waterMl / Math.max(goalMl, 1)) * 100, 100);
              return (
                <div key={label} className="flex-1 flex flex-col items-center gap-1.5">
                  <div className="w-full flex-1 bg-[#1E1E23] rounded-lg overflow-hidden flex flex-col justify-end">
                    <div
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-[#7A7A8C]">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm bg-[#0A84FF]" /> Partial
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm bg-[#30D158]" /> Goal met
            </div>
          </div>
        </Card>

        {/* Tips */}
        <Card className="p-4 border-[#0A84FF]/20 bg-[#0A84FF]/5">
          <p className="text-xs font-bold text-[#0A84FF] uppercase tracking-wider mb-2">Hydration Tips</p>
          <div className="space-y-2 text-sm text-[#7A7A8C]">
            <p>💧 Drink 500ml immediately after waking up</p>
            <p>🏋️ Add 500ml for every hour of intense exercise</p>
            <p>☀️ Add 250ml on hot days or high-sweat activities</p>
