      ...
      <div className="min-h-screen">
        ...
        <div className="relative h-64 sm:h-80 overflow-hidden">
          ...
          <button
            ...
            className="absolute top-4 left-4 sm:left-6 w-14 h-14 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white hover:bg-black/70 transition-colors"
          >
            <ArrowLeft size={18} />
          </button>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8">
            ...
            <p className="text-[#C8FF00] text-sm font-semibold mt-6">{plan.subtitle}</p>
          </div>
        </div>

        <div className="max-w-screen-xl mx-auto px-7 sm:px-6 py-6 space-y-6">
          {/* Stats row */}
          <div className="grid grid-cols-3 gap-5">
            {[
              ...
              <Card key={label} className="p-8 text-center">
                ...
                <p className="text-sm text-[#7A7A8C]">{label}</p>
          ...
          {/* Description */}
          <p className="text-sm text-[#B0B0BC] leading-relaxed">{plan.description}</p>
          ...
          <div className="flex gap-1 bg-[#17171A] border border-[#2A2A30] rounded-xl p-1">
            {[
              ...
              <button
                ...
                className={cn(
                  'flex-1 flex items-center justify-center gap-1.5 py-5.5 rounded-lg text-sm font-semibold transition-all',
                  activeTab === id
                    ? 'bg-[#C8FF00] text-[#0F0F11]'
                    : 'text-[#B0B0BC] hover:text-[#F0F0F5]'
                )}
              >
                {icon}
                {label}
...
function DayCard({ day, expanded, onToggle }: {
  ...
  return (
    <Card className={cn('overflow-hidden transition-all', expanded && 'border-[#C8FF00]/20')}>
      ...
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-6 p-7 text-left hover:bg-[#1E1E23] transition-colors"
      >
        {/* Day thumbnail */}
        ...
        <div className="flex-1 min-w-0">
          ...
          {day.type === 'rest' ? (
            <p className="text-sm text-[#30D158] mt-0.5">Active recovery day</p>
          ) : (
            <p className="text-sm text-[#B0B0BC] mt-0.5">{totalExercises} exercises · {day.blocks.length} blocks</p>
          )}
        </div>
      ...
      {expanded && (
        <div className="border-t border-[#2A2A30]">
          {day.blocks.map((block, bi) => (
            <div key={bi} className="p-7">
              {/* Block label */}
              <div
                className="inline-flex items-center gap-5 px-6 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider mb-4"
          ...
          {/* Finisher */}
          {day.finisher && (
            <div className="mx-4 mb-4 flex items-center gap-5 p-8 rounded-xl bg-[#C8FF00]/8 border border-[#C8FF00]/20">
              <span className="text-xl">🏁</span>
              <div>
                <p className="text-sm font-bold text-[#C8FF00] uppercase tracking-wider">Finisher</p>
...
function ExerciseCard({ exercise, blockColor }: { exercise: PlanExercise; blockColor: string }) {
  ...
  return (
    <div className="flex items-start gap-5 p-8 rounded-xl bg-[#1E1E23] hover:bg-[#252529] transition-colors">
      {/* Image */}
      ...
      <div className="flex-1 min-w-0">
        ...
        {exercise.note && (
          <p className="text-[11px] text-[#B0B0BC] mt-6.5 italic">💡 {exercise.note}</p>
        )}
        ...
        {exercise.videoUrl && (
          <a
            ...
            className="inline-flex items-center gap-1.5 mt-6 px-2.5 py-1 rounded-full text-[10px] font-bold transition-colors"
    ...
    <div className="space-y-6">
      {plan.meals.map((meal) => (
        <Card key={meal.id} className="overflow-hidden">
          {/* Meal header */}
          <div className="p-7 border-b border-[#2A2A30]">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-sm text-[#7A7A8C]">{meal.timing}</p>
              </div>
            </div>
            {meal.note && (
              <div
                className="mt-6 text-sm font-semibold px-6 py-5 rounded-xl"
                style={{
          ...
          <div className="p-7 space-y-5">
            <p className="text-[10px] font-bold text-[#B0B0BC] uppercase tracking-widest">Choose one option:</p>
            ...
            {meal.carbs && (
              <div className="mt-6">
                <p className="text-[10px] font-bold text-[#B0B0BC] uppercase tracking-widest mb-2">+ Choose one carb:</p>
                <div className="flex flex-wrap gap-5">
                  {meal.carbs.map((carb) => (
                    <span
                      key={carb}
                      className="text-sm px-6 py-1.5 rounded-full font-semibold bg-[#FF9F0A]/10 text-[#FF9F0A] border border-[#FF9F0A]/20"
                    >
                      {carb}
                    </span>
                  ))}
...
function FoodOptionCard({ option }: { option: { name: string; image: string; calories: number; protein: number } }) {
  ...
  return (
    <div className="flex items-center gap-5 p-8 rounded-xl bg-[#1E1E23] hover:bg-[#252529] transition-colors">
...
function RulesTab({ plan }: { plan: FitnessPlan }) {
  return (
    <div className="space-y-6">
      {/* Important rules */}
      <div>
        <p className="text-sm font-bold text-[#B0B0BC] uppercase tracking-wider mb-3">Important Rules</p>
        <div className="space-y-5">
          {plan.rules.map((rule) => (
            <div key={rule.title} className="flex gap-5 p-7 bg-[#17171A] border border-[#2A2A30] rounded-2xl">
              <span className="text-2xl shrink-0">{rule.icon}</span>
              <div>
                <p className="text-sm font-bold text-[#F0F0F5]">{rule.title}</p>
                <p className="text-sm text-[#B0B0BC] mt-6 leading-relaxed">{rule.body}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Supplements */}
      <div>
        <p className="text-sm font-bold text-[#B0B0BC] uppercase tracking-wider mb-3">
          Supplements <span className="text-[#4A4A5A] font-normal normal-case">(100% optional)</span>
        </p>
        <div className="grid grid-cols-2 gap-5">
          {plan.supplements.map((supp) => (
            <div key={supp.name} className="p-7 bg-[#17171A] border border-[#2A2A30] rounded-2xl">
              <div className="flex items-center gap-5 mb-2">
                <FlaskConical size={16} className="text-[#BF5AF2]" />
                <p className="text-sm font-bold text-[#F0F0F5]">{supp.name}</p>
              </div>
              <p className="text-sm text-[#7A7A8C]">{supp.dose}</p>
              <span className="inline-block mt-6 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#7A7A8C]/10 text-[#7A7A8C]">
                Optional
              </span>
            </div>
          ))}
      ...
      <Card className="p-8">
        <p className="text-sm font-bold text-[#B0B0BC] uppercase tracking-wider mb-4">Weekly Schedule</p>
        <div className="grid grid-cols-5 gap-5">
          {plan.days.map((day) => (
            <div
              key={day.day}
              className="flex flex-col items-center p-8 rounded-xl text-center"
              style={{
            ...
            >
              ...
              <p className="text-[9px] text-[#B0B0BC] font-semibold uppercase mt-0.5 leading-tight">
